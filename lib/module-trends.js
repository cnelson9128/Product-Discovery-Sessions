...
/*
 * Synthesizes a module-level trend — value created ("wow" quotes), feature
 * prioritization (now/later/future), adoption blockers, and go-to-market
 * messaging — from every already-analyzed session tagged to that module.
...
const TREND_SCHEMA = {
  ...
  required: ['overview_summary', 'value_moments', 'feature_prioritization', 'adoption_blockers', 'gtm_messaging'],
  additionalProperties: false
};
...
Produce: a short overview, the standout "value created" quotes/moments from these sessions, feature
prioritization split into now / later / future, a consolidated list of adoption blockers, and draft
...
- Never write a client or company name into any output text. You are given each session's client
  ...
  "overview_summary", any "rationale"/"impact"/"quote", or anywhere in "gtm_messaging". Refer to
...
- Every item in "value_moments", "now", "later", "future", and "adoption_blockers" must list the
  session ids that actually support it in "supporting_session_ids" — never include an item with
  zero supporting sessions, and never invent a session id that wasn't in the input.
- "value_moments" is built from each session's "value_created" statements and "one_sentence_pitch"
  ...
  statements. Unlike "now"/"later"/"future" below, a value moment does NOT need to recur across
  multiple sessions to count — one customer's specific, striking reaction is exactly what this
  field is for. Each value moment is exactly ONE person's reaction: "supporting_session_ids" must
  contain exactly the single session id that reaction came from — never combine reactions from
  different sessions into one moment, even if they're similar; list each as its own separate
  item instead. "quote" must paraphrase or quote what was actually said, cleaned of transcript
  noise, not restate "theme" in different words. If no session gave you a genuinely notable
  reaction — routine value statements alone don't count — return an empty list rather than
  stretching an ordinary comment into a "wow moment".
