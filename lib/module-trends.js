...
/*
 * Synthesizes a module-level trend — an overview, value created ("wow"
 * quotes), adoption blockers, and go-to-market messaging — from every
 * already-analyzed session tagged to that module. This is narrative
 * synthesis only: feature cards (the roadmap board) are sourced separately,
 * per item, classified by content via lib/raised-items.js +
 * lib/feature-sync.js, independent of which sessions are tagged to this
 * module — see lib/buckets.js's header for why this call used to also
 * generate "feature_prioritization" candidates and no longer does.
 * Takes each session's
...
/* A "wow" quote is one person's specific reaction — unlike the prioritization
   and adoption-blocker arrays below, which legitimately need to demonstrate
   recurrence across several sessions, a value moment must be attributable to
   exactly one session so the Quote Wall can show unambiguous attribution.
   Anthropic's structured-output schemas don't support `minItems`/`maxItems`
   on arrays (a real, deployed 400 error: "For 'array' type, property
   'maxItems' is not supported") — so "exactly one" is enforced by the
   ground rule in the system prompt below instead of the schema itself. The
   frontend's chips() helper still renders an array of any length correctly
   regardless, so a model slip-up here degrades gracefully rather than
   breaking anything. */
const oneSupportingId = supportedIds;
...
const TREND_SCHEMA = {
    ...
    value_moments: {
      ...
      items: {
        ...
        properties: {
          quote: { type: 'string' },
          theme: { type: 'string' },
          supporting_session_ids: oneSupportingId
        },
        required: ['quote', 'theme', 'supporting_session_ids'],
  ...
  required: ['overview_summary', 'value_moments', 'adoption_blockers', 'gtm_messaging'],
  additionalProperties: false
};
...
Produce: a short overview, the standout "value created" quotes/moments from these sessions, a
consolidated list of adoption blockers, and draft go-to-market messaging.
...
- Never write a client or company name into any output text. You are given each session's client
  ...
  "overview_summary", any "impact"/"quote", or anywhere in "gtm_messaging". Refer to
  sessions only via their "supporting_session_ids" — never inline a paraphrase like "one client
  said" that could be traced back to an individual customer's specific wording.
- Every item in "value_moments" and "adoption_blockers" must list the session ids that actually
  support it in "supporting_session_ids" — never include an item with zero supporting sessions,
  and never invent a session id that wasn't in the input.
- "value_moments" is built from each session's "value_created" statements and "one_sentence_pitch"
  — pull out genuinely notable "wow" reactions: specific, enthusiastic, surprised, or strongly
  validating language about what this module makes possible, not routine or lukewarm value
  statements. A value moment does NOT need to recur across multiple sessions to count — one
  customer's specific, striking reaction is exactly what this field is for. Each value moment is
  exactly ONE person's reaction: "supporting_session_ids" must contain exactly the single session
  id that reaction came from — never combine reactions from different sessions into one moment,
  even if they're similar; list each as its own separate item instead. "quote" must paraphrase or
  quote what was actually said, cleaned of transcript noise, not restate "theme" in different
  words. If no session gave you a genuinely notable reaction — routine value statements alone
  don't count — return an empty list rather than stretching an ordinary comment into a "wow
  moment".
