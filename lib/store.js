...
/* ---- login throttling ----
   Best-effort. Without a store linked this is a no-op, stated plainly rather
   than pretended otherwise — a shared password is the one secret this app
   has, so brute-forcing it is the important case to slow down. */
async function bumpFailures(ip, windowSeconds) {
  if (!configured()) return 0;
  try {
    const key = 'pds:login:fail:' + ip;
    const n = await command(['INCR', key]);
    if (n === 1) await command(['EXPIRE', key, windowSeconds]);
    return n;
  } catch (e) {
    return 0;
  }
...
async function countFailures(ip) {
  if (!configured()) return 0;
  try {
    const n = await command(['GET', 'pds:login:fail:' + ip]);
    return Number(n) || 0;
  } catch (e) {
    return 0;
  }
...
async function clearFailures(ip) {
  ...
  try {
    await command(['DEL', 'pds:login:fail:' + ip]);
  } catch (e) {
    /* nothing to do */
  }
}
...
/* ---- roadmap features ----
   One flat collection, not split into an index + per-record keys the way
   sessions are: a feature record carries no heavy payload (no transcript
   equivalent — just item/rationale/a handful of session ids/a few short
   fields), so every caller that reads this wants the full record anyway.
   Cards used to live nested inside pds:trend:<moduleId>.result.cards,
   regenerated wholesale on every rebuild; promoted here so they persist
   independently of any one trend build — get an owner, move between
   modules, track requirements/delivery status, and survive rebuilds. See
   api/module-trends-build.js (appends new features here, never touches an
   existing one) and api/features.js (the CRUD surface for everything else). */

const FEATURE_INDEX_KEY = 'pds:feature:index';

async function readFeatureIndex() {
  if (!configured()) return [];
  return (await getJSON(FEATURE_INDEX_KEY, [])) || [];
}

async function writeFeatureIndex(list) {
  await setJSON(FEATURE_INDEX_KEY, list);
}

/* ---- overall go-to-market messaging ----
   One record, not one per module — a company-wide synthesis across every
   analyzed session regardless of module, sitting a level above the
   per-module trends. Same shape and staleness convention as a module trend
   (see lib/module-trends.js's header), just keyed once instead of per
   module. */

const GTM_KEY = 'pds:gtm';

async function readGtm() {
  if (!configured()) return null;
  return getJSON(GTM_KEY, null);
}

async function writeGtm(record) {
  await setJSON(GTM_KEY, record);
}
