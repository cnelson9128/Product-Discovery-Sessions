 ...
 * session across all session types — the layer above lib/module-trends.js,
...
/* Generated from lib/modules.js rather than hand-listed here, so a module
   added or renamed there is reflected in this prompt automatically instead
   of silently drifting out of sync. */
const MODULE_LIST_PROSE = modules.MODULES.map(function (m) { return m.label; }).join(', ');
...
run across the whole research program so far — spanning all of its session types (${MODULE_LIST_PROSE}).
Each input record is one customer's already-extracted answers to the program's 11 standard discovery
questions for one session — not a raw transcript.
